...
# Since I want to put ffmpeg exe inside github's lib which is the hierachy nicer than put same path as duboku_gui.py
# then .spec runtime directory need same('.\\lib\\') as lib directory name AS run from command `python3 duboku_gui.py` bcoz they need refer same file(you don't want put duplicated ffmpeg exe in 2 places) which sahre the same code "lib" here.
ffmpeg_full_path = os.path.join(base_path, os.path.join('lib', ffmpeg_path) )
